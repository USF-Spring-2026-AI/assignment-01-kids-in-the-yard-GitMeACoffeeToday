...
family_tree.build_family_tree(person_list, dfh, master_list)
...
while user_input.lower() != 'q' and user_input.lower() != 'quit':
    ...
    if user_input.lower() == 't':
        print(f'This tree contains {family_tree.tree_counter([progenitor], dfh, 0)} people.')
        print('\n')
    elif user_input.lower() == 'd':
        result = family_tree.tree_name_by_decade([progenitor], dfh, {})
        for x in result:
            print(f'{x}: {result[x]}')
        print('\n')

    elif user_input.lower() == 'n':
        result = family_tree.tree_find_dup_names([progenitor], dfh, {})
        has_dup = False
        for x in result:
            if result[x] > 1:
                has_dup = True
